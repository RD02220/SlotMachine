...
/**
 * A slot machine usually has 3 reels which randomly land
 * on a symbol. This class includes an iterator.
 * 
 * @author Chad Golden
 *
 */
public class Reel implements Iterable<Symbol> {
	...
	/**
	 * Creates Reel from string where each character of
	 * the passed string becomes a symbol for the Reel.
	...
	/**
	 * Retrieves symbol from specified index. Retrieves
	 * in a circular fashion. E.g. if index passed is
	 * the last index, the symbol at index 0 will be
	 * returned.
	 * @param index
	...
	/**
	 * Returns randomly generated symbol from
	 * the collection of symbols contained in
	 * the reel.
	...
	/**
	 * Returns an array of three symbols when passed an
	 * index that serves as the central symbol.
	 * @return
	 */
	...
	@Override
	public void forEach(Consumer<? super Symbol> arg0) {
	...
	private class SymbolIterator implements Iterator<Symbol> {
		...
		public void forEachRemaining(Consumer arg0) {
			throw new UnsupportedOperationException("Method forEachRemaining() not supported.");
		}
		...
		@Override
		public Symbol next() {
			return (hasNext())	? symbols.get(currentIndex++) 
								: null;
		}
